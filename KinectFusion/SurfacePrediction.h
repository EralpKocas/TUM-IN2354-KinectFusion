...
public:
    ...
    bool gridInVolume(ImageProperties *&image_properties, Volume *&global_volume, Vector3f curr_grid) {
    ...
    float calculate_trilinear_interpolation(ImageProperties *&image_properties, Volume *&global_volume, Vector3f p) {
    ...
    void helper_compute_normal_map(ImageProperties *&image_properties, int level) {
        ...
        for (int i = 1; i < curr_height - 1; i++) {
            for (int j = 1; j < curr_width - 1; j++) {
                Vector3f curr_vertex = image_properties->all_data[level].vertex_map_predicted[j + i * curr_width];
                if (curr_vertex.z() == MINF || isnan(curr_vertex.z())) {
                    image_properties->all_data[level].normal_map_predicted[j + i * curr_width] = Vector3f(MINF, MINF,
                                                                                                          MINF);
                } else {
                    int pixel_y = i;
                    int pixel_x = j;
                    ...
                    image_properties->all_data[level].normal_map_predicted[j + i * curr_width] = cross_prod;
                }
            }
        }
    }

    void predict_surface(ImageProperties *&image_properties, Volume *&global_volume) {
        for (int level = 0; level < image_properties->num_levels; level++) {
            ...
            for (int j = 0; j < height; j++) {
                for (int i = 0; i < width; i++) {
                    ...
                    Vector3f pixel_ray = calculate_pixel_raycast(Vector3f(float(i + 0.5), float(j + 0.5), 1.f),
                                                                 ...
                                                                 image_properties->all_data[level].curr_fX,
                                                                 image_properties->all_data[level].curr_fY,
                                                                 image_properties->all_data[level].curr_cX,
                                                                 image_properties->all_data[level].curr_cY);
                    float camera_x = ((float) (i + 0.5) - image_properties->all_data[level].curr_cX) /
                                     image_properties->all_data[level].curr_fX;  // image to camera
                    float camera_y = ((float) (j + 0.5) - image_properties->all_data[level].curr_cY) /
                                     image_properties->all_data[level].curr_fY;  // image to camera
                    ...
                    for (float step = 0; step < max_ray_length; step += step_size * 0.5) {
                        ...
                        if (prev_tsdf > 0.f && curr_tsdf < 0.f)  // zero-crossing is found
                        {
                            ...
                            if (!gridInVolume(image_properties, global_volume, grid_location)) break;
            ...
            helper_compute_normal_map(image_properties, level);
        }
    }
